# Overlay port: a faithful copy of vcpkg's upstream raylib port (vcpkg/ports/raylib)
# with ONE intentional change — the build is pinned to SUPPORT_CUSTOM_FRAME_CONTROL=ON.
#
# Why: with that flag, raylib's EndDrawing() no longer swaps buffers or polls input
# on its own; bt::raylib::Renderer drives SwapScreenBuffer()/PollInputEvents()
# manually, and the Windows resize hook presents frames WITHOUT polling. Upstream
# only enables it implicitly (a side effect of -DCUSTOMIZE_BUILD=ON deriving the
# default from config.h), so a vcpkg update could silently flip it off. This overlay
# makes it explicit.
#
# Keep in sync with vcpkg/ports/raylib when the vcpkg submodule is updated. The only
# intended delta is the -DSUPPORT_CUSTOM_FRAME_CONTROL=ON option below.
...
vcpkg_cmake_configure(
    ...
    OPTIONS
        ...
        -DCUSTOMIZE_BUILD=ON
        # Overlay delta: expose and lock manual frame control on. With CUSTOMIZE_BUILD
        # this is a real CMake option; forcing it ON keeps EndDrawing() from swapping
        # buffers / polling input, which the app does itself.
